...
function dameCodigosEstados($id_estado)
{
	...
	$stid = oci_parse($conex, "SELECT DISTINCT * FROM FISC_CODIGOS_TELEFONO WHERE ID_ESTADO = :id_edo ORDER BY CODIGO_AREA");
	if (!$stid){
...
function dameDenunciasProceso()
{
	$resultado = false;
	$conex = conectaBaseDatos();
	$stid = oci_parse($conex, "SELECT 
		fd.ID_DENUNCIA, fd.ID_CIUDADANO, fd.ID_EMPRESA, fd.RIF AS RIF,
		(SELECT e.NOMBRE_FISC_EMPRESA FROM FISC_EMPRESA e WHERE e.FISC_EMPRESA=fd.FISC_EMPRESA)AS NOMBRE_EMPRESA
		FROM FISC_DENUNCIAS fd WHERE ESTATUS_DENUNCIA=0 AND ASIGNACION = 0");
	if (!$stid){
		$e = oci_error($this->conex);
		trigger_error(htmlentities($e['message'], ENT_QUOTES), E_USER_ERROR);
	}
...
function dameDirecciones($ids=null)
{
	...
	if($ids!=null) 
	{
		$consulta = "SELECT * FROM DIRECCIONES_ASIGNACION WHERE INTBORRADO=0 AND ID_DIRECCION <>1";
	}else
	{
...
function dameResponsable($id)
{
	
	$resultado = false;
	$conex = conectaBaseDatos();
	$consulta = "select nombre from direcciones_asignacion where ID_DIRECCION=:id";
	$stid = oci_parse($conex, $consulta);
	if (!$stid){
		$e = oci_error($this->conex);
		trigger_error(htmlentities($e['message'], ENT_QUOTES), E_USER_ERROR);
...
*/
	function dameDenunciasEspecificas()
	{
		$resultado = false;
		$conex = conectaBaseDatos();
		$stid = oci_parse($conex, "SELECT * FROM REPORTES_DENUNCIAS ORDER BY FECHA_DENUNCIA DESC");
		if (!$stid){
			$e = oci_error($this->conex);
			trigger_error(htmlentities($e['message'], ENT_QUOTES), E_USER_ERROR);
		}
	...
	*/
	...
	function dameQuejasProceso()
	{
		...
		$stid = oci_parse($conex, "SELECT 
			dj.ID_DENUNCIA, dj.ID_EMPRESA, dj.FECHA_DENUNCIA
			--(SELECT e.NOMBRE_FISC_EMPRESA FROM FISC_EMPRESA e WHERE e.ID_FISC_EMPRESA=dj.ID_EMPRESA and ROWNUM<=1)AS NOMBRE_EMPRESA

			FROM FISC_DENUNCIAS_JURIDICAS dj WHERE ESTATUS_DENUNCIA=0 AND ASIGNADA = 0");
	...
	function dameQuejasPanelReporte()
	{
		$resultado = false;
		$conex = conectaBaseDatos();
		$stid = oci_parse($conex, "SELECT * FROM FISC_DENUNCIAS_JURIDICAS dj ORDER BY dj.FECHA_DENUNCIA DESC");
	...
	function dameDivisiones($id)
	{
		$resultado = false;

		$conex = conectaBaseDatos();	

		$consulta ="SELECT * FROM TBL_DIVISIONES WHERE ID_DIRECCION_LINEA =:id";
	...
	function dameRoles()
	{
		$resultado = false;
		$conex = conectaBaseDatos();
		$stid = oci_parse($conex, "SELECT * FROM TBL_ROLES ORDER BY CLV_ROL");
		if (!$stid){
			$e = oci_error($this->conex);
			trigger_error(htmlentities($e['message'], ENT_QUOTES), E_USER_ERROR);
		}
	...
	function dameNombreEstado($id)
	{
		$resultado = false;
		$conex = conectaBaseDatos();
		$consulta = "SELECT NOMBRE_ESTADO FROM SIRA.ESTADO WHERE ID_ESTADO=:id";
		$stid = oci_parse($conex, $consulta);
		if (!$stid)
		{
			$e = oci_error($this->conex);
		...
		// Cierra la conexión Oracle
		oci_close($conex);
		$estado = $resultado[0]['NOMBRE_ESTADO']; 
		return $estado;
	}


	function dameNombreMunicipio($id)
	{
		$resultado = false;
		$conex = conectaBaseDatos();
		$consulta = "SELECT NOMBRE_MUNICIPIO FROM SIRA.MUNICIPIO WHERE ID_MUNICIPIO=:id";
		$stid = oci_parse($conex, $consulta);
		if (!$stid)
		{
			$e = oci_error($this->conex);
		...
		// Cierra la conexión Oracle
		oci_close($conex);
		$municipio = $resultado[0]['NOMBRE_MUNICIPIO']; 
		return $municipio;
	}


	function dameNombreParroquia($id)
	{
		$resultado = false;
		$conex = conectaBaseDatos();
		$consulta = "SELECT DESC_PARROQUIA FROM SIRA.PARROQUIA WHERE ID_PARROQUIA=:id";
		$stid = oci_parse($conex, $consulta);
		if (!$stid)
		{
			$e = oci_error($this->conex);
		...
		// Cierra la conexión Oracle
		oci_close($conex);
		$parroquia = $resultado[0]['DESC_PARROQUIA']; 
		return $parroquia;
	}
