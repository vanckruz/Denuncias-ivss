...
<body>
	...
	<div style="width:100%;background:white;min-height:600px;padding:25px;margin-bottom:75px;"><!--Content-->
		...
		<form action="../mod_denuncias/Controllers/Controller_Denuncia_Correo.php " method="post" id="form_denuncias_asig">
			...
			<table class="table table-hover" id="listadoDenuncias">
				...
				<tbody class="tbody_list_den">
					...
					<tr>
						<td><input type="checkbox" class="check_denuncias" id="denuncia_true" name="denuncia_true[]"  value="<?=$den['ID_DENUNCIA'];?>" style="cursor:pointer;"></td>
						<td><?=$den['ID_DENUNCIA'];?></td>
						<td><?=$den['NOMBRE_EMPRESA'];?></td>
						<td style="width:100px;"><?=( $den['ID_EMPRESA'] = "N/A" ? $den['RIF'] : $den['ID_EMPRESA']);?></td>
						<td>
							<ol style="margin-left:10px;text-align:left;">
								<?php
								if( dameNombreMotivos($den['ID_DENUNCIA']) != null){
	...
	<script>
		$(document).on("ready",function(){
			...
			$("#enviar_form_asignacion").on("click",function(){
				...
				if(($('input[name="denuncia_true[]"]')).is(':checked')){
					boo=true;
				}else
				{
					if(boo==true) boo=false;
					errores = "Se debe seleccionar como mínimo una Denuncia";
				...
				{
					if(boo==true) boo=false;
					errores = "Seleccione mínimo una Dirección de Envío";
