...
<head>
	...
	<title>Registro de Motivos</title>
...
<body>
	...
	<div style="width:100%;margin:auto;margin-bottom:0px;background:transparent;color:white;padding:7px 0px 0px 0px;position:relative;border-bottom:1px dotted white;" id="titulo_gestor_motivos">
		<div style="background:#f1f1f1;border-bottom:1px dotted black;padding:7px;overflow:hidden">
			<h3 style="float:left;color:black;padding:7px;">Gestión de documentos de denuncia</h3>
			<button class="btn btn-primary" style="float:right;" id="btn_agregar_motivos"><span class="glyphicon glyphicon-plus-sign"></span> Agregar Documentos</button>
		</div>
	</div>
		...
		<div style="width:100%;">
			...
			<table class="table table-hover" id="tabla_motivos">
				...
				foreach ($documentos as $documento) {
					echo '<tr>
					<td style="padding:16px !important;">'.$documento["DESCRIPCION"].'</td>
					<td>
						<button id="'.$documento["ID_DOCUMENTO"].'" class="btn btn-primary mas_user edit_mot_denuncia" data-nombre_motivo="'.str_replace(" ","-",$documento["DESCRIPCION"]).'">
							<span class="glyphicon glyphicon-pencil"></span>
						</button>
					</td>
					<td>
						<button id="'.$documento["ID_DOCUMENTO"].'" class="btn btn-danger mas_user eliminar_mot_denuncia" data-nombre_motivo="'.str_replace(" ","-",$documento["DESCRIPCION"]).'">
	...
	<div style="width:70%;margin:auto;height:auto;overflow:hidden;padding:25px;background:#f1f1f1;position:relative;">
		<form action="Controllers/Controller_Documentos_Denuncias.php" method="post" id="form_agregar_motivo">
			...
			<div id="input_agregador_motivos" style="margin-top:35px">
				...
				<input type="text" class="form-control inputivo input_agregar" style="border-radius:0px;" name="documento[]" maxlength="200">					
			</div>
			<hr>
			<div>
...
<div id="editar_motivo" style="display:none;width:100%;height:100%;position:fixed;top:0px;left:0px;z-index:50;overflow:hidden;">
	<div style="width:100%;margin-bottom:25px;background:#3B5998;color:white;padding:26px;">
		<h1 style="text-align:center;">Editar este Documento</h1>
	...
	<div style="width:70%;margin:auto;height:250px;overflow:hidden;padding:25px;background:#f1f1f1;">
		<form action="Controllers/Controller_Documentos_Denuncias.php" method="post" id="form_editar_motivo">
			<p style="text-align:left;margin-bottom:12px;font-weight:bold;">Descripción del documento:</p>
			<input type="hidden" name="option_editar" value="editar">
			<input type="hidden" name="id_motivo" id="id_mots" >
			<input type="text" class="form-control" style="border-radius:0px;" name="descripcion_documento" id="input_motivo_edicion"  maxlength="200">
			<hr>
			<div>
...
<div id="eliminar_motivo" style="display:none;width:100%;height:100%;position:fixed;top:0px;left:0px;z-index:50;overflow:hidden;">
	<div style="width:100%;margin-bottom:25px;background:#3B5998;color:white;padding:26px;">
		<h1 style="text-align:center;">Eliminar este Documento</h1>
		<!--<span class="glyphicon glyphicon-remove-circle" style="float:right;font-size:1.6em;margin-bottom:21px;margin-right:21px;cursor:pointer;"></span>-->
	</div>	
	<div style="width:50%;margin:auto;height:200px;overflow:hidden;padding:12px;background:#f1f1f1;">
		<form action="Controllers/Controller_Documentos_Denuncias.php" method="post" id="form_eliminar_motivo">
			<p style="text-align:center;margin-bottom:12px;font-size:1.1em;font-weight:bold;">¿Desea eliminar este documento?</p>
			<hr>
...
<!--Modal confirmar-->
<div id="confirm_motivo" style="display:none;width:100%;height:100%;position:fixed;top:0px;left:0px;z-index:50;background:rgba(0,0,0,0.5);overflow:hidden;">
	<div style="width:50%;margin:auto;height:200px;overflow:hidden;padding:12px;background:#f1f1f1;">
		<form action="Controllers/Controller_Documentos_Denuncias.php" method="post" id="form_eliminar_motivo">
			<p style="text-align:center;margin-bottom:12px;font-size:1.1em;font-weight:bold;" id="mensaje_confirm"></p>
			<hr>

			<div>
				<button class="btn btn-danger" id="submit_confirm"><span class="glyphicon glyphicon-trash"></span> Aceptar</button>
				<button class="btn btn-default" id="cancelar_confirm"><span class="glyphicon glyphicon-remove-circle"></span> Cancelar</button>
			</div>
		</form>	
	</div>
</div>
	...
	$(document).on("ready",function(){
		...
		$("#submit_agregar").on("click",function(a){
			...
			if(boo == $(".input_agregar").length ){
				...
				$("#cancelar_confirm").on("click",function(e){
					e.preventDefault();
					$("#confirm_motivo").hide();	
					$(".input_agregar").each(function(index, el) {
						$(el).val("");
...
/*Function agregar*/
function agregar(){
	var miembro='<input type="text" class="form-control inputivo input_agregar" style="border-radius:0px;margin-top:7px;" name="documento[]" maxlength="200">';
