<?php 
	...
	// Se crea el objeto PHPExcel
		...
		$titulosColumnas = array('DENUNCIA','NOMBRE DEL DENUNCIANTE','CEDULA','DESCRIPCION','FECHA DENUNCIA','EMPRESA','N° PATRONAL','ESTATUS','DIRECCION','FECHA ASIGNADA','MOTIVOS','FECHA CIERRE'); 
		...
		{	
			...
			foreach ($denuncias as $key => $fila) 
			{
	            ...
	            ->setCellValue('B'.$i, $fila['DATOS_CIUDADANO'])
        		...
        		->setCellValue('G'.$i, $fila['ID_EMPRESA'])
        		->setCellValue('H'.$i, $fila['TIPO'])
        		->setCellValue('I'.$i, $nombre)
        		->setCellValue('J'.$i, $fecha)
        		->setCellValue('K'.$i, $fila['MOTIVOS'])
		        ->setCellValue('L'.$i, $fila['FECHA_CIERRE']);
