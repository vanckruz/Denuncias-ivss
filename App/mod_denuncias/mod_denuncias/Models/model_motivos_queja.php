...
class MotivoQuejaDAO
{
	...
	{	
		...
		$stid = oci_parse($this->conex, "SELECT * FROM FISC_MOTIVOS_QUEJAS");
		if (!$stid){
	...
	**/
	...
	{	
		$this->conex = DataBase::getInstance();
		$stid = oci_parse($this->conex, "SELECT * FROM FISC_MOTIVOS_QUEJAS WHERE ID_MOTIVO=:id_mot");
		if (!$stid){
    		$e = oci_error($this->conex);
    		trigger_error(htmlentities($e['message'], ENT_QUOTES), E_USER_ERROR);
		}
	...
	{
		$this->conex = DataBase::getInstance();
		$consulta = "DELETE FROM FISC_MOTIVOS_QUEJAS WHERE ID_MOTIVO = :id_mot";
	...
	*
	**/
	public function editar(&$motivo)
	{
		$id_mot      = $motivo->__GET('id_motivo');
		$descripcion = $motivo->__GET('descripcion');

		$this->conex = DataBase::getInstance();
		$stid = oci_parse($this->conex, "UPDATE FISC_MOTIVOS_QUEJAS SET 
	...
	*
	**/
public function agregar(&$motivo)
{
	$this->conex  = DataBase::getInstance();
	$consulta = "INSERT INTO FISC_MOTIVOS_QUEJAS (
			ID_MOTIVO, 
 			DESCRIPCION)
			values
			(
	...
	/*********************************************************/
	//Actualizar el próximo id_motivo en la tabla ID_TABLAS
	/*********************************************************/
